...
int ft_check_flags(char *argv[], t_list *stack_a, t_list *stack_b, struct data *b)
{
	int i;
	int ii;
...
int	main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			b->disorder = ft_find_disorder(stack_a); //CALCUL DISORDER STACK_A									 //
			if (b->disorder == 0)
			{
				// ft_lstclear(&stack_a);
				// ft_lstclear(&stack_b);
			...
			{
				printf("\n");
			 	stack_a = ((ft_adaptive(stack_a, stack_b, b)));           //SI NON, ALGO ADAPTIF   
			}
